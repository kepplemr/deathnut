...
dnr_client = DeathnutRestClient(service='test', failure_callback=execute_me_on_failure, redis_connection=fake_redis_conn)
...
class TestRestDeathnutClient(unittest.TestCase):
    ...
    def test_execute_if_authorized(self):
        ...
        self.assertFalse(dnr_client.execute_if_authorized('test_user', 'own', random_resource_id, True, True, False, execute_me_on_success))
        dnr_client.assign_role('test_user', 'own', random_resource_id)
        self.assertTrue(dnr_client.execute_if_authorized('test_user', 'own', random_resource_id, True, True, False, execute_me_on_success))
    
    @patch.object(dnr_client, '_is_authorized', new=slow_auth)
    def test_dont_wait_speeds_up(self):
        random_resource_id = str(uuid.uuid4())
        start = timer()
        self.assertTrue(dnr_client.execute_if_authorized('test_user', 'own', random_resource_id, True, True, False, slow_getter))
        end = timer()
        wait_time = int(end - start)
        start = timer()
        self.assertTrue(dnr_client.execute_if_authorized('test_user', 'own', random_resource_id, True, True, True, slow_getter))
        end = timer()
