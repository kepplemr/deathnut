...
dn_client = DeathnutClient(
    service="test", resource_type="recipes", redis_connection=fake_redis_conn
)
...
class TestBaseDeathnutClient(unittest.TestCase):
    ...
    def test_assign_role(self):
        ...
        self.assertTrue(
            fake_redis_conn.hget("test_recipes:test_user:own", random_resource_id)
        )
    ...
    def test_get_resources(self):
        ...
        self.assertEqual(
            5, len(list(dn_client.get_resources("test_user", "view", page_size=20)))
        )
        self.assertEqual(
            9, len(list(dn_client.get_resources("test_user", "view", page_size=10)))
        )
        self.assertEqual(
            1, len(list(dn_client.get_resources("test_user", "view", page_size=90)))
        )
